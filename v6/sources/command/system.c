...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/15 11:43:26 by joaribei          #+#    #+#             */
/*   Updated: 2022/08/16 19:14:35 by joaribei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*_command_pre_system(t_node_command *invoker, t_node_command *this)
{
	...
	if (!_command_token(&name, this->next))
	{
		...
		name = ft_strjoin(name, this->next->builting->pre_function(invoker, this->next));
	}
	else
...
void _command_system(t_node_command *this)
{
	...
	{
		name = ft_strjoin(this->name, " ");
		name = ft_strjoin(name, this->next->builting->pre_function(this, this->next));
	}
	else
		name = _string().dup(this->name);
	if (this)
	{
		...
		if (c_pid == 0)
		{
			...
			paths = _string().split_char(_env().get("PATH")->value, ':');
			while (args[test])
				test++;
			args[test] = NULL;
			if (execve(get_cmd(paths, this->name), args, _shell()->env))
				if (_string().length(this->name) > 0)
